...
#' @param Resistance A \code{SpatRaster} resistance surface, or a numeric vector of length 2 giving \code{c(min, max)} values.
...
#' Because of the flexibility of the Ricker function to take a monomolecular
#' shape (try \code{Plot.trans(PARM=c(10,100), Resistance=c(1,10),
#' transformation="Ricker")} to see this), whenever a shape parameter >6 is
#' selected in combination with a Ricker family transformation, the
#' transformation reverts to a Distance transformation. In most applications,
#' monomolecular families should be preferred and Ricker families should be
#' explored only when there is a strong biological or ecological rationale for
#' a unimodal or quadratic-type relationship. Otherwise, the extra flexibility
#' of Ricker curves can increase the risk of overfitting. This constraint has
#' not been implemented in the \code{Plot.trans} function.
...
Plot.trans <- function(PARM,
  ...
  if (length(Resistance) == 2) {
    ...
    stop("`Resistance` must be a SpatRaster or a numeric vector of length 2.")
  }

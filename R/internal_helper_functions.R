  ...
  function(PARM,
    ...
    if (!is.null(iter)) {
      if (GA.inputs$surface.type[iter] == "cat") {
        ...
        parm <- PARM
        df <-
          data.frame(id = terra::unique(r)[[1]], PARM) # Data frame with original raster values and replacement values
    ...
    r <- terra::classify(r, matrix(c(-Inf, 1e-06, 1e-06, 1e6, Inf, 1e6), ncol = 3, byrow = TRUE))
...
# Rescale a numeric vector to [MIN, MAX]
...
# Rescale a SpatRaster to [MIN, MAX]
SCALE <- function(data, MIN, MAX, threshold = 1e-5) {
  if (abs(MIN - MAX) < threshold) {
    vals <- terra::values(data)
    vals[is.finite(vals)] <- 0
    terra::values(data) <- vals
    return(data)
  }
  Mn <- terra::global(data, "min", na.rm = TRUE)[[1]]
  Mx <- terra::global(data, "max", na.rm = TRUE)[[1]]
  if (abs(Mx - Mn) < threshold) {
    terra::values(data) <- MIN
    return(data)
  }
  (MAX - MIN) / (Mx - Mn) * (data - Mx) + MAX
}
...
sv.cont.nG <- function(direction,
  ...
  for (r in 1:pop.size) {
    if (runif(1) < .5 && direction == "Increase") {
      ...
      z <- c(sample(eqs, 1), runif(1, .01, 10), runif(1, 1, max))
    }
    cont.starts[r, ] <- z
  }
  cont.starts
...
# Helper: get unique raster values as a numeric vector (terra-compatible)
rast_unique <- function(r) {
  as.numeric(terra::unique(r)[[1]])
}
...
Result.txt <-
  function(GA.results,
    ...
    if(inherits(GA.results, 'ga')) {
      ...
      cat(paste0(" --- ResistanceGA2 summary output ---"), "\n")
      cat("\n")
